...
category: benchmark
aliases: ['/docs-cn/stable/benchmark/how-to-run-tpcc/']
---
...
因为该型号 CPU 是 NUMA 架构，建议用 `numactl` 进行绑核。首先[安装 numactl](/production-deployment-using-tiup.md/#如何安装-numactl-工具) ，然后用 `lscpu` 查看 NUMA node，比如：
